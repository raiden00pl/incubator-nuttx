...
/* This is the lower-half implementation for the Nuttx FOC device.
 *
 ...
 * at vector 0 (all low-side switched are on and the current flows through
 * current sensors).
 *
 ...
 *     - common interrupt for ADC1 and ADC2
 *
 ...
 * conversion.
 *
 * Note that a driver based on float32 calculations will not work correctly
 * on chips without FPU support or with FPU support disabled.
 *
...
/* For float calculations the FPU must be supported */

#ifdef CONFIG_POWER_FOC_USE_FLOAT
#  ifndef CONFIG_ARCH_FPU
#    error "CONFIG_POWER_FOC_USE_FLOAT reguires FPU support"
#  endif
#endif
...
#if CONFIG_POWER_FOC_INST > 1
#  if defined(CONFIG_STM32_HAVE_IP_ADC_V2)
#    error Not tested yet
#  endif
#endif

/* FOC model not supported yet */

#ifdef CONFIG_POWER_FOCMODEL
#  error FOC model not supported yet
#endif
...
#    if FOC0_PWM_NCHANNELS != (CONFIG_POWER_FOC_PHASES + 1)
#      error Invalid channels configuration
#    endif
#  endif
#  if defined(CONFIG_STM32_FOC_FOC1)
#    if FOC1_PWM_NCHANNELS != (CONFIG_POWER_FOC_PHASES + 1)
#      error Invalid channels configuration
#    endif
#  endif
...
#  if defined(CONFIG_STM32_FOC_FOC0)
#    if FOC0_PWM_NCHANNELS != (CONFIG_POWER_FOC_PHASES)
#      error Invalid channels configuration
#    endif
#  endif
#  if defined(CONFIG_STM32_FOC_FOC1)
#    if FOC1_PWM_NCHANNELS != (CONFIG_POWER_FOC_PHASES)
#      error Invalid channels configuration
#    endif
#  endif
...
{
  foc_number_t   curr[CONFIG_POWER_FOC_PHASES];        /* Current */
  uint8_t        work_div;                             /* FOC work prescaler */
  ...
  uint32_t       curr_offset[CONFIG_POWER_FOC_PHASES]; /* ADC current offset */
  int16_t        curr_raw[CONFIG_POWER_FOC_PHASES];    /* ADC current RAW */
};
...
static int stm32_foc_fault_clean(FAR struct foc_dev_s *dev);
...
static int stm32_foc_work_cfg(FAR struct foc_dev_s *dev, uint32_t freq);
static int stm32_foc_notifier_cfg(FAR struct foc_dev_s *dev,
                                  uint32_t freq);
...
static struct stm32_foc_dev_s  g_stm32_foc_dev[CONFIG_POWER_FOC_INST];
static struct stm32_foc_priv_s g_stm32_foc_priv[CONFIG_POWER_FOC_INST];
...
static struct foc_lower_ops_s g_stm32_foc_ops =
{
  ...
  .bind           = stm32_foc_bind,
  .fault_clean    = stm32_foc_fault_clean,
#ifdef CONFIG_POWER_FOC_TRACE
...
static struct foc_lower_s g_stm32_foc_lower[CONFIG_POWER_FOC_INST];
...
void stm32_foc_sync_all(void)
{
  ...
  uint32_t                    egr_reg[CONFIG_POWER_FOC_INST];
  int                         i       = 0;
...
static int stm32_foc_notifier_cfg(FAR struct foc_dev_s *dev, uint32_t freq)
{
  ...
  DEBUGASSERT(priv);
  DEBUGASSERT(freq > 0);
  DEBUGASSERT(priv->data.adc_freq > 0);

  /* Validate input:
   *   1. must be fraction of ADC interrupt frequency
   */

  if (priv->data.adc_freq % freq)
    {
      ret = -EINVAL;
      goto errout;
    }

  /* Get notifier divider */

  priv->data.notifier_div = (priv->data.adc_freq / freq);
                               ...
                               FAR struct foc_cfgcmn_s *cfg)
{
  ...
  ret = stm32_foc_work_cfg(dev, cfg->work_freq);
  if (ret < 0)
    {
      pwrerr("ERROR: stm32_foc_work_cfg failed %d\n", ret);
      goto errout;
    }
...
#elif defined(CONFIG_STM32_FOC_ADC_TRGO)
  stm32_foc_adc_trgo_trg_set(dev, (dev->cfgcmn.pwm_freq /
                                   priv->data.adc_freq));
#else
#  error Invalid FOC ADC trigger
#endif
...
static int stm32_foc_setup(FAR struct foc_dev_s *dev)
{
  ...
  /* Call board-specific setup - must be done before calibration start */
  ...
  if (priv->data.adcint_cntr < CAL_SAMPLES)
    {
      ...
      for (i = 0; i < CONFIG_POWER_FOC_SHUNTS; i += 1)
        {
      ...
      if (dev != NULL)
        {
          ...
          /* Do we make it before next interrupt? */

          pending = STM32_ADC_INT_GET(adc);
          if ((pending & ADC_ISR_FOC) != 0)
            {
              /* Ignore for the first interrupts that may not meet
               * time requirements.
               * REVISIT:
               */

              if (priv->data.adcint_cntr > 10)
                {
                  /* Raise ARCH fault */

                  dev->state.fault.s.fault |= FOC_FAULT_ARCH;
                }
            }
...
static int stm32_foc_worker_handler(FAR struct foc_dev_s *dev)
{
  ...
  /* Is it time for a FOC work ? */

  if (priv->data.adcint_cntr % priv->data.work_div == 0)
    {
      for (i = 0; i < CONFIG_POWER_FOC_SHUNTS; i += 1)
        {
      ...
      board->ops->current_get(dev, priv->data.curr_raw, priv->data.curr);

      /* Call upper-half worker callback */

      DEBUGASSERT(priv->cb->foc_work);
      priv->cb->foc_work(dev, priv->data.curr);
    }

  /* Is it time for a FOC notifier ? */

  if (priv->data.adcint_cntr % priv->data.notifier_div == 0)
    {
      /* Call upper-half notifier callback */

      DEBUGASSERT(priv->cb->notifier);
      priv->cb->notifier(dev);
    }
...
static int stm32_foc_calibration_start(FAR struct foc_dev_s *dev)
{
  ...
  stm32_foc_adc_ccr4_trg_set(dev, (priv->data.per - ADC_TRIGGER_OFFSET));
#elif defined(CONFIG_STM32_FOC_ADC_TRGO)
  stm32_foc_adc_trgo_trg_set(dev, 0);
#else
#  error Invalid FOC ADC trigger
#endif
                                  ...
                                  FAR foc_number_t *duty)
{
  FAR struct stm32_foc_priv_s *priv    = STM32_FOC_PRIV_FROM_DEV_GET(dev);
  FAR struct stm32_foc_dev_s  *foc_dev = STM32_FOC_DEV_FROM_DEV_GET(dev);
  uint16_t                     ccr[CONFIG_POWER_FOC_PHASES];
  ...
  switch (dev->info.ftype)
    {
      ...
      case FOC_NUMBER_TYPE_FLOAT:
        {
          DEBUGASSERT(duty[0].f32 >= 0.0f);
          DEBUGASSERT(duty[1].f32 >= 0.0f);
#if CONFIG_POWER_FOC_PHASES > 2
          DEBUGASSERT(duty[2].f32 >= 0.0f);
#endif
#if CONFIG_POWER_FOC_PHASES > 3
          DEBUGASSERT(duty[3].f32 >= 0.0f);
#endif

          ccr[0] = (uint16_t)(duty[0].f32 * priv->data.per);
          ccr[1] = (uint16_t)(duty[1].f32 * priv->data.per);
#if CONFIG_POWER_FOC_PHASES > 2
          ccr[2] = (uint16_t)(duty[2].f32 * priv->data.per);
#endif
#if CONFIG_POWER_FOC_PHASES > 3
          ccr[3] = (uint16_t)(duty[3].f32 * priv->data.per);
#endif
      ...
      case FOC_NUMBER_TYPE_FIXED16:
        {
          DEBUGASSERT(duty[0].b16 >= 0);
          DEBUGASSERT(duty[1].b16 >= 0);
#if CONFIG_POWER_FOC_PHASES > 2
          DEBUGASSERT(duty[2].b16 >= 0);
#endif
#if CONFIG_POWER_FOC_PHASES > 3
          DEBUGASSERT(duty[3].b16 >= 0);
#endif

          ccr[0] = (uint16_t)b16toi(b16muli(duty[0].b16, priv->data.per));
          ccr[1] = (uint16_t)b16toi(b16muli(duty[1].b16, priv->data.per));
#if CONFIG_POWER_FOC_PHASES > 2
          ccr[2] = (uint16_t)b16toi(b16muli(duty[2].b16, priv->data.per));
#endif
#if CONFIG_POWER_FOC_PHASES > 3
          ccr[3] = (uint16_t)b16toi(b16muli(duty[3].b16, priv->data.per));
#endif
  ...
  switch (dev->info.ftype)
    {
      ...
      case FOC_NUMBER_TYPE_FLOAT:
        {
          DEBUGASSERT(board->data->pwm_dt_ns > 0);

          lower->hw_cfg.pwm_dt_ns   = board->data->pwm_dt_ns;
          lower->hw_cfg.pwm_max.f32 = board->data->duty_max.f32;
      ...
      case FOC_NUMBER_TYPE_FIXED16:
        {
          ...
          lower->hw_cfg.pwm_dt_ns   = board->data->pwm_dt_ns;
          lower->hw_cfg.pwm_max.b16 = board->data->duty_max.b16;
...
static int stm32_foc_work_cfg(FAR struct foc_dev_s *dev, uint32_t freq)
{
  ...
  priv->data.work_div = (dev->cfgcmn.pwm_freq / freq);

  /* ADC interrupts frequency is PWM frequency */

  priv->data.adc_freq = dev->cfgcmn.pwm_freq;

#elif defined(CONFIG_STM32_FOC_ADC_TRGO)

  /* Call work on every ADC interrupt */

  priv->data.work_div = 1;

  /* ADC interrupts frequency is work frequency */
...
static int stm32_foc_fault_clean(FAR struct foc_dev_s *dev)
{
  ...
  return board->ops->fault_clean(dev);
}
...
stm32_foc_initialize(int inst, FAR struct stm32_foc_board_s *board)
{
  ...
  DEBUGASSERT(board->ops->fault_clean);
