...
#    if FOC0_PWM_NCHANNELS != (CONFIG_DRIVERS_FOC_PHASES + 1)
#      error Invalid channels configuration
#    endif
#  endif
#  if defined(CONFIG_STM32_FOC_FOC1)
#    if FOC1_PWM_NCHANNELS != (CONFIG_DRIVERS_FOC_PHASES + 1)
#      error Invalid channels configuration
#    endif
#  endif
...
#  if defined(CONFIG_STM32_FOC_FOC0)
#    if FOC0_PWM_NCHANNELS != (CONFIG_DRIVERS_FOC_PHASES)
#      error Invalid channels configuration
#    endif
#  endif
#  if defined(CONFIG_STM32_FOC_FOC1)
#    if FOC1_PWM_NCHANNELS != (CONFIG_DRIVERS_FOC_PHASES)
#      error Invalid channels configuration
#    endif
#  endif
...
{
  foc_current_t curr[CONFIG_DRIVERS_FOC_PHASES];        /* Current */
  ...
  uint32_t      curr_offset[CONFIG_DRIVERS_FOC_SHUNTS]; /* ADC current offset */
  int16_t       curr_raw[CONFIG_DRIVERS_FOC_SHUNTS];    /* ADC current RAW */
};
...
#ifdef CONFIG_DRIVERS_FOC_TRACE
...
static struct stm32_foc_dev_s  g_stm32_foc_dev[CONFIG_DRIVERS_FOC_INST];
static struct stm32_foc_priv_s g_stm32_foc_priv[CONFIG_DRIVERS_FOC_INST];
...
static struct foc_lower_s g_stm32_foc_lower[CONFIG_DRIVERS_FOC_INST];

/* FOC upper-half device data */

static struct foc_dev_s g_foc_dev[CONFIG_DRIVERS_FOC_INST];
...
void stm32_foc_sync_all(void)
{
  ...
  uint32_t                    egr_reg[CONFIG_DRIVERS_FOC_INST];
  int                         i       = 0;

  /* Get registers to write */

  for (i = 0; i < CONFIG_DRIVERS_FOC_INST; i += 1)
    {
      /* Get FOC device */
...
#if CONFIG_DRIVERS_FOC_PHASES > 2
  ...
  if (priv->data.adcint_cntr < CAL_SAMPLES)
    {
      ...
      for (i = 0; i < CONFIG_DRIVERS_FOC_SHUNTS; i += 1)
        {
                                  ...
                                  FAR foc_duty_t *duty)
{
  FAR struct stm32_foc_priv_s *priv    = STM32_FOC_PRIV_FROM_DEV_GET(dev);
  FAR struct stm32_foc_dev_s  *foc_dev = STM32_FOC_DEV_FROM_DEV_GET(dev);
  uint16_t                     ccr[CONFIG_DRIVERS_FOC_PHASES];
                          ...
                          FAR struct foc_callbacks_s *cb)
{
  ...
  if (dev->devno > CONFIG_DRIVERS_FOC_INST)
    {
