...
#ifndef __INCLUDE_NUTTX_DRIVERS_FOC_FOC_H
#define __INCLUDE_NUTTX_DRIVERS_FOC_FOC_H
...
/* Phase current as signed 32-bit int */

typedef int32_t foc_current_t;

/* Phase duty cycle as unsigned fixed16.
 * REVISIT: poor resolution for fixed16
 */
...
struct foc_state_s
{
  ...
  foc_current_t curr[CONFIG_DRIVERS_FOC_PHASES]; /* Phase current feedback */
};
...
struct foc_params_s
{
  foc_duty_t duty[CONFIG_DRIVERS_FOC_PHASES];  /* PWM duty cycle for phases */
};
...
#endif /* __INCLUDE_NUTTX_DRIVERS_FOC_FOC_H */
