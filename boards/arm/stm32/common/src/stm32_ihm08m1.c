/****************************************************************************
 * boards/arm/stm32/common/src/stm32_foc_ihm08m1.c
 *
...
/* Voltage reference for ADC - we assume 3.3V.
...
#define IPHASE_RATIO (-19.3f)
#define IPHASE_ADC   (ADC_TO_VOLT * IPHASE_RATIO)
#define IPHASE_MAX   (15.0f)
...
static struct stm32_foc_board_data_s g_stm32_foc_board_data_f32 =
{
...
static struct stm32_foc_board_s g_stm32_foc_board_f32 =
{
  .data = &g_stm32_foc_board_data_f32,
...
static struct stm32_foc_board_data_s g_stm32_foc_board_data_b16 =
{
  .adc_cfg          = NULL,     /* board-specific */
...
static struct stm32_foc_board_s g_stm32_foc_board_b16 =
{
  .data = &g_stm32_foc_board_data_b16,
  .ops  = &g_stm32_foc_board_ops,
};
  ...
  switch (dev->info.ftype)
    {
      ...
      case FOC_NUMBER_TYPE_FLOAT:
        {
          curr[0].f32 = curr_raw[0] * IPHASE_ADC;
          curr[1].f32 = curr_raw[1] * IPHASE_ADC;
          curr[2].f32 = curr_raw[2] * IPHASE_ADC;
      ...
      case FOC_NUMBER_TYPE_FIXED16:
        {
          curr[0].b16 = b16muli(ftob16(IPHASE_ADC), curr_raw[0]);
          curr[1].b16 = b16muli(ftob16(IPHASE_ADC), curr_raw[1]);
          curr[2].b16 = b16muli(ftob16(IPHASE_ADC), curr_raw[2]);
...
#ifdef CONFIG_BOARD_STM32_IHM08M1_FLOAT
/****************************************************************************
 * Name: board_ihm08m1_f32_initialize
 ****************************************************************************/

int board_ihm08m1_f32_initialize(FAR struct stm32_foc_adc_s *adc_cfg)
{
  ...
  if (g_foc_dev == NULL)
    {
      ...
      g_stm32_foc_board_data_f32.adc_cfg = adc_cfg;

      /* Initialize arch specific FOC lower-half */

      foc = stm32_foc_initialize(0, &g_stm32_foc_board_f32);
      if (foc == NULL)
        {
          ret = -errno;
      ...
      ret = focf32_register(FOC_DEVPATH, foc);
      if (ret < 0)
        {
...
#endif  /* CONFIG_BOARD_STM32_IHM08M1_FLOAT */

#ifdef CONFIG_BOARD_STM32_IHM08M1_FIXED16
/****************************************************************************
 * Name: board_ihm08m1_b16_initialize
 ****************************************************************************/

int board_ihm08m1_b16_initialize(FAR struct stm32_foc_adc_s *adc_cfg)
{
  FAR struct foc_dev_s *foc         = NULL;
  int                   ret         = OK;
  ...
  if (g_foc_dev == NULL)
    {
      ...
      g_stm32_foc_board_data_b16.adc_cfg = adc_cfg;

      /* Initialize arch specific FOC lower-half */

      foc = stm32_foc_initialize(0, &g_stm32_foc_board_b16);
      if (foc == NULL)
        {
          ret = -errno;
          pwrerr("ERROR: failed to initialize STM32 FOC: %d\n", ret);
      ...
      ret = focb16_register(FOC_DEVPATH, foc);
      if (ret < 0)
        {
          printf("ERROR: failed to register FOC device: %d\n", ret);
          goto errout;
...
#endif  /* CONFIG_BOARD_STM32_IHM08M1_FIXED16 */
