 ...
 *   Supported IOCTLs:
 *
 *   FOCIOC_START:        Start the FOC device,
 *                        arg: none
 *
 *   FOCIOC_STOP:         Stop the FOC device,
 *                        arg: none
 *
 *   FOCIOC_GET_STATE:    Get the FOC device state,
 *                        arg: struct foc_state_s pointer
 *                        This is a blocking operation that is used to
 *                        synchronize the user space application with
 *                        a FOC worker.
 *
 *   FOCIOC_CLEAN_FAULT:  Clean the FOC device fault state,
 *                        arg: none
 *
 *   FOCIOC_SET_PARAMS:   Set the FOC device operation parameters,
 *                        arg: struct foc_params_s pointer
 *
 *   FOCIOC_GET_PARAMS:   Get the FOC device operation parameters,
 *                        arg: struct foc_params_s pointer
 *
 *   FOCIOC_SET_CONFIG:   Set the FOC device configuration,
 *                        arg: struct foc_cfg_s pointer
 *
 *   FOCIOC_GET_CONFIG:   Get the FOC device configuration,
 *                        arg: struct foc_cfg_s pointer
 *
 *   FOCIOC_GET_INFO:     Get the FOC device info,
      ...
      case FOCIOC_START:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_START failed %d\n", ret);
            }
      ...
      case FOCIOC_STOP:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_STOP failed %d\n", ret);
            }
      ...
      case FOCIOC_GET_STATE:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_GET_STATE failed %d\n", ret);
            }
      ...
      case FOCIOC_CLEAN_FAULT:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_CLEAN_FAULT failed %d\n", ret);
            }
      ...
      case FOCIOC_SET_PARAMS:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_SET_PARAMS failed %d\n", ret);
            }
      ...
      case FOCIOC_SET_CONFIG:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_SET_CONFIG failed %d\n", ret);
            }
      ...
      case FOCIOC_GET_CONFIG:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_GET_CONFIG failed %d\n", ret);
            }
      ...
      case FOCIOC_GET_INFO:
        {
          ...
          if (ret != OK)
            {
              pwrerr("ERROR: FOCIOC_GET_INFO failed %d\n", ret);
            }
...
#ifdef CONFIG_DRIVERS_FOC_HAVE_CORDIC
...
static int foc_stop(FAR struct foc_dev_s *dev)
{
  foc_duty_t d_zero[CONFIG_DRIVERS_FOC_PHASES];
  int        ret = OK;
  ...
  memset(&d_zero, 0, CONFIG_DRIVERS_FOC_PHASES * sizeof(foc_duty_t));
...
static int foc_cfg_get(FAR struct foc_dev_s *dev, FAR struct foc_cfg_s *cfg)
{
  ...
  memcpy(cfg, &dev->cfg, sizeof(struct foc_cfg_s));
                        ...
                        FAR foc_current_t *current)
{
  ...
  memcpy(&dev->state.curr,
         current,
         sizeof(foc_current_t) * CONFIG_DRIVERS_FOC_PHASES);
